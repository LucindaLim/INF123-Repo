...
import pygame
from pygame.locals import KEYDOWN, QUIT, K_ESCAPE, K_w, K_a, K_d, K_s, K_q, K_SPACE


############################################################

def process_input(prev_game_status, prev_direction):
    (game_status, direction) = (prev_game_status, prev_direction)
...
def draw_everything(screen, mybox, pellets, borders):
    ...
    [pygame.draw.rect(screen, (200, 11, 20), b) for b in borders]  # # Deep Sky Blue
    [pygame.draw.rect(screen, (25, 132, 203), p) for p in pellets]  # pink
    pygame.draw.rect(screen, (5, 200, 200), mybox)  # Deep Sky Blue
...
def create_borders(dims, thickness=2):
    ...
    return [pygame.Rect(0, 0, thickness, h),
            ...
            pygame.Rect(50, 40, 100, thickness),
            pygame.Rect(100, 200, thickness, 50)]
